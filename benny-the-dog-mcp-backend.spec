...
# PyInstaller spec — produce benny-the-dog-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller benny-the-dog-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/benny_the_dog_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/benny_the_dog_mcp", "benny_the_dog_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
...
exe = EXE(
    ...
    a.datas,
    [],
    name="benny-the-dog-mcp-backend",
    debug=False,
